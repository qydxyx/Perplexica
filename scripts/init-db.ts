...
const tablesInfo = db
  ...
  .all();
const chatTableExists = tablesInfo.some((table: any) => table.name === 'chats');

if (chatTableExists) {
  // Check if user_id column exists in chats table
  const chatsColumns = db.prepare('PRAGMA table_info(chats)').all();
  const hasUserIdColumn = chatsColumns.some(
